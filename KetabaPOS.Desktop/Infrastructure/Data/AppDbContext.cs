...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Product>(e =>
        {
            ...
            e.Property(p => p.RetailPrice).HasPrecision(18, 2);
            e.Property(p => p.WholesalePrice).HasPrecision(18, 2);
        ...
        modelBuilder.Entity<Sale>(e =>
        {
            ...
            e.HasOne(s => s.User).WithMany(u => u.Sales).HasForeignKey(s => s.UserId);
        });

        modelBuilder.Entity<SaleItem>(e =>
        {
            e.Property(si => si.UnitPrice).HasPrecision(18, 2);
            e.Property(si => si.DiscountAmount).HasPrecision(18, 2);
            e.Property(si => si.TotalPrice).HasPrecision(18, 2);
            e.HasOne(si => si.Sale).WithMany(s => s.SaleItems).HasForeignKey(si => si.SaleId);
        ...
        modelBuilder.Entity<PurchaseItem>(e =>
        {
            ...
            e.HasOne(pi => pi.Purchase).WithMany(p => p.PurchaseItems).HasForeignKey(pi => pi.PurchaseId);
            e.HasOne(pi => pi.Product).WithMany(p => p.PurchaseItems).HasForeignKey(pi => pi.ProductId);
        });

        modelBuilder.Entity<InventoryTransaction>(e =>
        {
            e.Property(it => it.Type).HasConversion<string>().HasMaxLength(20);
            e.HasOne(it => it.Product).WithMany(p => p.InventoryTransactions).HasForeignKey(it => it.ProductId);
        });
        ...
        modelBuilder.Entity<Loan>(e =>
        {
            ...
            e.Property(l => l.LoanType).HasConversion<string>().HasMaxLength(20);
            e.Property(l => l.Status).HasConversion<string>().HasMaxLength(20);
            e.HasOne(l => l.Customer).WithMany(c => c.Loans).HasForeignKey(l => l.CustomerId);
            e.HasOne(l => l.Supplier).WithMany(s => s.Loans).HasForeignKey(l => l.SupplierId);
        });

        modelBuilder.Entity<LoanPayment>(e =>
        {
            e.Property(lp => lp.Amount).HasPrecision(18, 2);
            e.HasOne(lp => lp.Loan).WithMany(l => l.LoanPayments).HasForeignKey(lp => lp.LoanId);
        });
        ...
        {
            e.HasQueryFilter(bl => !bl.IsDeleted);
        });
    }
}
