...
public class ProductService : IProductService
{
    ...
    public async Task<IEnumerable<Product>> GetProductsAsync(string? search = null, int? categoryId = null, int page = 1, int pageSize = 50)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search)) query = query.Where(p => p.Name.Contains(search) || p.NameAr.Contains(search) || p.Barcode.Contains(search) || p.SKU.Contains(search));
        if (categoryId.HasValue) query = query.Where(p => p.CategoryId == categoryId.Value);
        return await query.OrderBy(p => p.Name).Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();
    }
    public async Task<Product?> GetByBarcodeAsync(string barcode) => await _context.Products.FirstOrDefaultAsync(p => p.Barcode == barcode);
    ...
    public async Task UpdateAsync(Product product) { product.UpdatedAt = DateTime.UtcNow; _context.Products.Update(product); await _context.SaveChangesAsync(); }
    public async Task DeleteAsync(int id)
    {
        var product = await _context.Products.FindAsync(id);
        if (product != null) { product.IsDeleted = true; product.UpdatedAt = DateTime.UtcNow; await _context.SaveChangesAsync(); }
    }
