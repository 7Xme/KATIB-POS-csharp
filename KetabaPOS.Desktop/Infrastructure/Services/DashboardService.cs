...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardData> GetDashboardDataAsync()
    {
        ...
        var salesData = await _context.Sales
            .Where(s => s.CreatedAt >= DateTime.UtcNow.Date.AddDays(-6) && s.Status == SaleStatus.Completed)
        ...
        {
            var point = last7Days.FirstOrDefault(d => d.Label == data.Date.ToString("ddd"));
            if (point != null)
                point.Value = data.Total;
        }
        ...
        var activities = recentSales.Select(s => new RecentActivity
        {
            Description = $"Sale #{s.InvoiceNumber} - {s.TotalAmount:F2} by {s.User?.DisplayName ?? "N/A"}",
